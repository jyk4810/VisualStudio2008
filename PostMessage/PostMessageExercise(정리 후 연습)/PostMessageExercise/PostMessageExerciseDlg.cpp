...
// PostMessageExerciseDlg.cpp : 구현 파일
//

#include "stdafx.h"
#include "PostMessageExercise.h"
#include "PostMessageExerciseDlg.h"
#include "ChildDlg.h"
...
// CPostMessageExerciseDlg 대화 상자




CPostMessageExerciseDlg::CPostMessageExerciseDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CPostMessageExerciseDlg::IDD, pParent)
	, m_Height(800)
	, m_Width(600)
	, m_OffsetX(0)
	, m_OffsetY(0)
...
void CPostMessageExerciseDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(CPostMessageExerciseDlg, CDialog)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_BUTTON_CHILDDIALOG, &CPostMessageExerciseDlg::OnBnClickedButtonChilddialog)
	ON_MESSAGE(USER_MESSAGE_DELETE_DIALOG, OnUserMessageDeleteDialog)
	ON_MESSAGE(USER_MESSAGE_SET_VALUE, OnUserMessageIntegerNameValue)
END_MESSAGE_MAP()


// CPostMessageExerciseDlg 메시지 처리기

BOOL CPostMessageExerciseDlg::OnInitDialog()
{
...
void CPostMessageExerciseDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CPostMessageExerciseDlg::OnPaint()
{
	if (IsIconic())
	{
...
//  이 함수를 호출합니다.
HCURSOR CPostMessageExerciseDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}


void CPostMessageExerciseDlg::OnBnClickedButtonChilddialog()
{
	if( m_pChildDlg == NULL )
	{
...
BOOL CPostMessageExerciseDlg::PreTranslateMessage(MSG* pMsg)
{	
...
LRESULT CPostMessageExerciseDlg::OnUserMessageDeleteDialog(WPARAM wParam, LPARAM lParam)
{
	delete m_pChildDlg;
	m_pChildDlg = NULL;
...
LRESULT CPostMessageExerciseDlg::OnUserMessageIntegerNameValue(WPARAM wParam, LPARAM lParam)
{	
	PSTRUCT pStruct = (PSTRUCT)lParam;
	switch(wParam)
	{
	case VALUE_TYPE_INTEGER:
		SetDlgItemText(IDC_STATIC_INTEGERNAME, pStruct->Name);
		SetDlgItemText(IDC_STATIC_INTEGERVALUE, pStruct->Value);	
		break;
	case VALUE_TYPE_FLOAT:
		SetDlgItemText(IDC_STATIC_FLOATNAME, pStruct->Name);
		SetDlgItemText(IDC_STATIC_FLOATVALUE, pStruct->Value);	
		break;
	case VALUE_TYPE_ENUMERATION:
		SetDlgItemText(IDC_STATIC_ENUMNAME, pStruct->Name);
		SetDlgItemText(IDC_STATIC_ENUMVALUE, pStruct->Value);	
		break;
	case VALUE_TYPE_COMMAND:
		SetDlgItemText(IDC_STATIC_COMNAME, pStruct->Name);
		SetDlgItemText(IDC_STATIC_COMVALUE, pStruct->Value);	
		break;
	case VALUE_TYPE_STRING:
		SetDlgItemText(IDC_STATIC_STRNAME, pStruct->Name);
		SetDlgItemText(IDC_STATIC_STRVALUE, pStruct->Value);	
		break;
	case VALUE_TYPE_BOOLEAN:
		SetDlgItemText(IDC_STATIC_BOOLNAME, pStruct->Name);
		SetDlgItemText(IDC_STATIC_BOOLVALUE, pStruct->Value);	
		break;
	default:			
		break;	
	}				
...
CString CPostMessageExerciseDlg::GetParam(int type, CString strName)
{
	CString strTemp = _T("");
...
void CPostMessageExerciseDlg::SetParam(int type, CString strName, CString strVal)
{
	switch(type)
	{
	case 0:	//int
...
CString CPostMessageExerciseDlg::GetIntValue(CString strName)
{
	CString strTemp = _T("");
	int iVal = 0;
...
void CPostMessageExerciseDlg::SetIntValue(CString strName, CString strVal)
{
	if(strName == "Width")
	{
...
CString CPostMessageExerciseDlg::GetFloValue(CString strName)
{
	CString strVal;
	float iVal = 0.0f;
...
void CPostMessageExerciseDlg::SetFloValue(CString strName, CString strVal)
{
	if(strName == "TriggerDelay")
	{
...
CString CPostMessageExerciseDlg::GetEnumValue(CString strName)
{
	CString strTemp = _T("");
	int iVal = 0;
...
void CPostMessageExerciseDlg::SetEnumValue(CString strName, CString strVal)
{
	if(strName == "DeviceScanType")
	{
...
CString CPostMessageExerciseDlg::GetComValue(CString strName)
{
	CString strTemp = _T("");
	int iVal = 0;
...
void CPostMessageExerciseDlg::SetComValue(CString strName, CString strVal)
{
	if(strName == "AcquisitionStart")
	{
...
CString CPostMessageExerciseDlg::GetStrValue(CString strName)
{
	CString strTemp = _T("");
	int iVal = 0;
...
void CPostMessageExerciseDlg::SetStrValue(CString strName, CString strVal)
{
	if(strName == "DeviceID")
	{
...
CString CPostMessageExerciseDlg::GetBooValue(CString strName)
{
	CString strTemp = _T("");
	int iVal = 0;
...
void CPostMessageExerciseDlg::SetBooValue(CString strName, CString strVal)
{
	if(strName == "LineInverter")
	{
